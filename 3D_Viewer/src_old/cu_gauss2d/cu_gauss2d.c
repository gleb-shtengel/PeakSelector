...
*/
int calc_num_segments(int nimgs, int *segsize, int *card1, int *card2) {
    struct cudaDeviceProp prop;
    size_t tot_mem, scale, sys_mem;
    int segs, ncards, ntesla;
    size_t totalGlobalMem, a;
    
    //  Find the Tesla cards and set the total memory per card
    ...
    scale = 11*11*sizeof(unsigned short) + 
            ...
            (6+6+1)*sizeof(float);
    tot_mem = nimgs * scale;
    sys_mem = (size_t)(0.9*totalGlobalMem) * ntesla;
...
*  idl_gauss2d
*/
static IDL_VPTR idl_gauss2d(int argc, IDL_VPTR *argv) {
    IDL_VPTR ans;
    int imax, n, i, idx;
    IDL_MEMINT dims;
    unsigned short *imgs;
...
#endif
            if (n != 0) {
                gauss2d(&imgs[idx], n, &seeds[i*segsize], n, constraints, imax, &out[(6+6+1)*i*segsize], uncertainties);
            }
        }
    }
...
int IDL_CDECL IDL_Load(void) {
    ...
    static IDL_SYSFUN_DEF2 function_addr[] = {  
        {{(IDL_FUN_RET) idl_gauss2d}, "CU_GAUSS2D", 4, 4, 0, 0},
    };  
