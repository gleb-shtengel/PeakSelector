...
del cu_filterit_f.dll
del cu_filterit_f.obj
del filterit_f.obj
...
nvcc -m64 -ccbin %CC% -I%CUDA_INC_PATH% -I%CUDA_UTIL_PATH% -O3 --use_fast_math -DWIN32 -c filterit_f.cu

REM  Build the DLM obj file
cl -nologo -I"%IDL_DIR%\external\include" -I"%CUDA_INC_PATH%" -DWIN32 -c cu_filterit_f.c

REM  Now link everything together
link /DLL /OUT:cu_filterit_f.dll /DEF:cu_filterit_f.def "%CUDA_LIB_PATH%\cuda.lib" "%CUDA_LIB_PATH%\cudart.lib" "%CUDA_LIB_PATH%\cufft.lib" "%CUDA_LIB_PATH%\cublas.lib" "%IDL_LIBDIR%\idl.lib" filterit_f.obj cu_filterit_f.obj
