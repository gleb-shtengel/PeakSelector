...
*  file:  cu_filter_f.c
*
*  Single precision DLM code for GPU GroupFilterIt and FilterIt.
*
...
*/
extern void gpfilter(float *gp, int npeaks, float *params, 
                     unsigned char *out);
extern void filter(float *gp, int npeaks, float *params, 
                   unsigned char *out);
...
static IDL_VPTR idl_gpfilter(int argc, IDL_VPTR *argv) {
  ...
  gp = (float *)argv[0]->value.arr->data;
...
int IDL_CDECL IDL_Load(void) {
  ...
  static IDL_SYSFUN_DEF2 function_addr[] = {  
    {{(IDL_FUN_RET) idl_gpfilter}, "CU_GROUPFILTERIT_F", 2, 2, 0, 0},
    {{(IDL_FUN_RET) idl_filter}, "CU_FILTERIT_F", 2, 2, 0, 0},
  };  

  /*  Error messages  */
  if (!(msg_block = IDL_MessageDefineBlock("FILTER_F", IDL_CARRAY_ELTS(msg_arr), msg_arr))) {
    return IDL_FALSE;  
  }
...
//dlm: MODULE CU_FILTER_F
//dlm: DESCRIPTION CUDA GroupFilterIt and FilterIt (single), HHMI
//dlm: VERSION 1.0
//dlm: FUNCTION CU_GROUPFILTERIT_F 2 2
//dlm: FUNCTION CU_FILTERIT_F 2 2
