...
;-----------------------------------------------------------------------------
;
; gpuUNARY_OP_NAME (generated from gpuUnaryOp.pro.sed)
;
; Template for binary operations on GPU. Used to auto-generate binary
; operation procedures using sed.
; Substitution tokens are: UNARY_OP, BINARY_OP_NAME and BINARY_OP_TEXT
;
...
;
;   Result = p1 * UNARY_OP(p2 * p3 + p4) + p5
;
...
;       error status
;-
function gpuUNARY_OP_NAME, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    1: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = UNARY_OP(*x.data)
         endif else begin
           case result_type of
             4 : err = gpuUNARY_OP_NAMEF(x.n_elements, x.handle, _lhs.handle)
             5 : err = gpuUNARY_OP_NAMED(x.n_elements, x.handle, _lhs.handle)
             6 : err = gpuUNARY_OP_NAMEC(x.n_elements, x.handle, _lhs.handle)
             9 : err = gpuUNARY_OP_NAMEZ(x.n_elements, x.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = p1 * UNARY_OP(p2 * *x.data + p4) + p5
         endif else begin
           case result_type of
             4 : err = gpuUNARY_OP_NAMEFAT(x.n_elements, $
                                           float(p1), $
                                           float(p2), x.handle, float(p4), $
                                           float(p5), $
                                           _lhs.handle)
             5 : err = gpuUNARY_OP_NAMEDAT(x.n_elements, $
                                           double(p1), $
                                           double(p2), x.handle, double(p4), $
                                           double(p5), $
                                           _lhs.handle)
             6 : err = gpuUNARY_OP_NAMECAT(x.n_elements, $
                                           complex(p1), $
                                           complex(p2), x.handle, complex(p4), $
                                           complex(p5), $
                                           _lhs.handle)
             9 : err = gpuUNARY_OP_NAMEZAT(x.n_elements, $
                                           dcomplex(p1), $
    ...
    else: message, level=-1, 'gpuUNARY_OP_NAME: incorrect number of arguments'
  endcase
...
;       error status
;-
pro gpuUNARY_OP_NAME, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  on_error, 2

  case n_params() of
    2: p2 = gpuUNARY_OP_NAME(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                             LHS=p2, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuUNARY_OP_NAME(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
