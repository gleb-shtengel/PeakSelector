...
;       error status
;-
function gpuDiv, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    2: begin
         ...
         if ((x.n_elements ne y.n_elements)) then begin   
           ...
           message, level=-1, 'gpuDiv: input vector length missmatch'
         endif
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = *x.data / *y.data
           ...
           case result_type of
             4 : err = gpuDivF(x.n_elements, x.handle, y.handle, _lhs.handle)
             5 : err = gpuDivD(x.n_elements, x.handle, y.handle, _lhs.handle)
             6 : err = gpuDivC(x.n_elements, x.handle, y.handle, _lhs.handle)
             9 : err = gpuDivZ(x.n_elements, x.handle, y.handle, _lhs.handle)
           endcase
         endelse
...
pro gpuDiv, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  ...
  case n_params() of
    3: p3 = gpuDiv(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                              arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
                              LHS=p3, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuDiv(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                              arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
