...
;-----------------------------------------------------------------------------
;
; gpuGt.pro (generated from gpuRelationalOp.pro.sed)
;
; Template for relational operations on GPU. Used to auto-generate binary
; operation procedures using sed.
; Substitution tokens are: GT, Gt and Compare two vetors for GT
;
...
;       error status
;-
function gpuGt, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    2: begin
         ...
         if (x.n_elements ne y.n_elements) then begin
           ...
           message, level=-1, 'gpuGt: input vector length missmatch'
         endif

         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(*x.data GT *y.data)
         endif else begin
           case result_type of
             4 : err = gpuGtF(x.n_elements, x.handle, y.handle, _lhs.handle)
             5 : err = gpuGtD(x.n_elements, x.handle, y.handle, _lhs.handle)
             6 : err = gpuGtC(x.n_elements, x.handle, y.handle, _lhs.handle)
             9 : err = gpuGtZ(x.n_elements, x.handle, y.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(p1 * *x.data GT p3 * *y.data + p5)
         endif else begin
           case result_type of
             4 : err = gpuGtFAT(x.n_elements, $
                                         float(p1), x.handle, $
                                         float(p3), y.handle, $
                                         float(p5), $
                                         _lhs.handle)
             5 : err = gpuGtDAT(x.n_elements, $ 
                                         double(p1), x.handle, $
                                         double(p3), y.handle, $
                                         double(p5), $
                                         _lhs.handle)
             6 : err = gpuGtCAT(x.n_elements, $
                                         complex(p1), x.handle, $
                                         complex(p3), y.handle, $
                                         complex(p5), $
                                         _lhs.handle)
             9 : err = gpuGtZAT(x.n_elements, $
...
pro gpuGt, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  ...
  case n_params() of
    3: p3 = gpuGt(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
                           LHS=p3, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuGt(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
