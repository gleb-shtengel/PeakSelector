...
;-----------------------------------------------------------------------------
;
; gpuLt.pro (generated from gpuRelationalOp.pro.sed)
;
; Template for relational operations on GPU. Used to auto-generate binary
; operation procedures using sed.
; Substitution tokens are: LT, Lt and Compare two vetors for LT
;
...
;       error status
;-
function gpuLt, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    2: begin
         ...
         if (x.n_elements ne y.n_elements) then begin
           ...
           message, level=-1, 'gpuLt: input vector length missmatch'
         endif

         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(*x.data LT *y.data)
         endif else begin
           case result_type of
             4 : err = gpuLtF(x.n_elements, x.handle, y.handle, _lhs.handle)
             5 : err = gpuLtD(x.n_elements, x.handle, y.handle, _lhs.handle)
             6 : err = gpuLtC(x.n_elements, x.handle, y.handle, _lhs.handle)
             9 : err = gpuLtZ(x.n_elements, x.handle, y.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(p1 * *x.data LT p3 * *y.data + p5)
         endif else begin
           case result_type of
             4 : err = gpuLtFAT(x.n_elements, $
                                         float(p1), x.handle, $
                                         float(p3), y.handle, $
                                         float(p5), $
                                         _lhs.handle)
             5 : err = gpuLtDAT(x.n_elements, $ 
                                         double(p1), x.handle, $
                                         double(p3), y.handle, $
                                         double(p5), $
                                         _lhs.handle)
             6 : err = gpuLtCAT(x.n_elements, $
                                         complex(p1), x.handle, $
                                         complex(p3), y.handle, $
                                         complex(p5), $
                                         _lhs.handle)
             9 : err = gpuLtZAT(x.n_elements, $
...
pro gpuLt, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  ...
  case n_params() of
    3: p3 = gpuLt(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
                           LHS=p3, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuLt(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
