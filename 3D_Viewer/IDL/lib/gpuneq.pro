...
;-----------------------------------------------------------------------------
;
; gpuNeq.pro (generated from gpuRelationalOp.pro.sed)
;
; Template for relational operations on GPU. Used to auto-generate binary
; operation procedures using sed.
; Substitution tokens are: NE, Neq and Compare two vetors for NEQ
;
...
;       error status
;-
function gpuNeq, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    2: begin
         ...
         if (x.n_elements ne y.n_elements) then begin
           ...
           message, level=-1, 'gpuNeq: input vector length missmatch'
         endif

         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(*x.data NE *y.data)
         endif else begin
           case result_type of
             4 : err = gpuNeqF(x.n_elements, x.handle, y.handle, _lhs.handle)
             5 : err = gpuNeqD(x.n_elements, x.handle, y.handle, _lhs.handle)
             6 : err = gpuNeqC(x.n_elements, x.handle, y.handle, _lhs.handle)
             9 : err = gpuNeqZ(x.n_elements, x.handle, y.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(p1 * *x.data NE p3 * *y.data + p5)
         endif else begin
           case result_type of
             4 : err = gpuNeqFAT(x.n_elements, $
                                         float(p1), x.handle, $
                                         float(p3), y.handle, $
                                         float(p5), $
                                         _lhs.handle)
             5 : err = gpuNeqDAT(x.n_elements, $ 
                                         double(p1), x.handle, $
                                         double(p3), y.handle, $
                                         double(p5), $
                                         _lhs.handle)
             6 : err = gpuNeqCAT(x.n_elements, $
                                         complex(p1), x.handle, $
                                         complex(p3), y.handle, $
                                         complex(p5), $
                                         _lhs.handle)
             9 : err = gpuNeqZAT(x.n_elements, $
    ...
    else: message, level=-1, 'gpuNeq: incorrect number of arguments'
  endcase
...
;       error status
;-
pro gpuNeq, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  on_error, 2


  case n_params() of
    3: p3 = gpuNeq(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
                           LHS=p3, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuNeq(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
