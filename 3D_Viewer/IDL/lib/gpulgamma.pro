...
;-----------------------------------------------------------------------------
;
; gpuLgamma (generated from gpuUnaryOp.pro.sed)
;
; Template for binary operations on GPU. Used to auto-generate binary
; operation procedures using sed.
; Substitution tokens are: lngamma, BINARY_OP_NAME and BINARY_OP_TEXT
;
...
;       error status
;-
function gpuLgamma, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    1: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = lngamma(*x.data)
         endif else begin
           case result_type of
             4 : err = gpuLgammaF(x.n_elements, x.handle, _lhs.handle)
             5 : err = gpuLgammaD(x.n_elements, x.handle, _lhs.handle)
             6 : err = gpuLgammaC(x.n_elements, x.handle, _lhs.handle)
             9 : err = gpuLgammaZ(x.n_elements, x.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = p1 * lngamma(p2 * *x.data + p4) + p5
         endif else begin
           case result_type of
             4 : err = gpuLgammaFAT(x.n_elements, $
                                           float(p1), $
                                           float(p2), x.handle, float(p4), $
                                           float(p5), $
                                           _lhs.handle)
             5 : err = gpuLgammaDAT(x.n_elements, $
                                           double(p1), $
                                           double(p2), x.handle, double(p4), $
                                           double(p5), $
                                           _lhs.handle)
             6 : err = gpuLgammaCAT(x.n_elements, $
                                           complex(p1), $
                                           complex(p2), x.handle, complex(p4), $
                                           complex(p5), $
                                           _lhs.handle)
             9 : err = gpuLgammaZAT(x.n_elements, $
                                           dcomplex(p1), $
...
pro gpuLgamma, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  ...
  case n_params() of
    2: p2 = gpuLgamma(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                             LHS=p2, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuLgamma(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
