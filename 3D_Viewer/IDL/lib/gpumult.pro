...
;-----------------------------------------------------------------------------
;
; gpuMult.pro (generated from gpuBinaryOp.pro.sed)
;
; Template for binary operations on GPU. Used to auto-generate binary
; operation procedures using sed.
; Substitution tokens are: *, Mult and Multiply two vectors
;
...
;       error status
;-
function gpuMult, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    2: begin
         ...
         if ((x.n_elements ne y.n_elements)) then begin   
           ...
           message, level=-1, 'gpuMult: input vector length missmatch'
         endif

         if (!gpu.mode eq 0) then begin
           *_lhs.data = *x.data * *y.data
         endif else begin
           case result_type of
             4 : err = gpuMultF(x.n_elements, x.handle, y.handle, _lhs.handle)
             5 : err = gpuMultD(x.n_elements, x.handle, y.handle, _lhs.handle)
             6 : err = gpuMultC(x.n_elements, x.handle, y.handle, _lhs.handle)
             9 : err = gpuMultZ(x.n_elements, x.handle, y.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = (p1 * *x.data) * (p3 * *y.data) + p5
         endif else begin
           case result_type of
             4 : err = gpuMultFAT(x.n_elements, $
                                            float(p1), $
                                            x.handle, $
                                            float(p3), $
                                            y.handle, $
...
pro gpuMult, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  ...
  case n_params() of
    3: p3 = gpuMult(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                              arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
                              LHS=p3, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuMult(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                              arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
