...
;-----------------------------------------------------------------------------
;
; gpuRint (generated from gpuUnaryOp.pro.sed)
;
...
;       error status
;-
function gpuRint, p1, p2, p3, p4, p5, LHS=lhs, $
    ...
    1: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = round(*x.data)
           ...
           case result_type of
             4 : err = gpuRintF(x.n_elements, x.handle, _lhs.handle)
             5 : err = gpuRintD(x.n_elements, x.handle, _lhs.handle)
             6 : err = gpuRintC(x.n_elements, x.handle, _lhs.handle)
             9 : err = gpuRintZ(x.n_elements, x.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = p1 * round(p2 * *x.data + p4) + p5
           ...
           case result_type of
             4 : err = gpuRintFAT(x.n_elements, $
                                           float(p1), $
                                           float(p2), x.handle, float(p4), $
                                           float(p5), $
                                           _lhs.handle)
             5 : err = gpuRintDAT(x.n_elements, $
                                           double(p1), $
                                           double(p2), x.handle, double(p4), $
                                           double(p5), $
                                           _lhs.handle)
             6 : err = gpuRintCAT(x.n_elements, $
                                           complex(p1), $
                                           complex(p2), x.handle, complex(p4), $
                                           complex(p5), $
                                           _lhs.handle)
             9 : err = gpuRintZAT(x.n_elements, $
                                           dcomplex(p1), $
...
pro gpuRint, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  ...
  case n_params() of
    2: p2 = gpuRint(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                             LHS=p2, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuRint(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
