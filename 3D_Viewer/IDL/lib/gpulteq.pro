...
;-----------------------------------------------------------------------------
;
; gpuLtEq.pro (generated from gpuRelationalOp.pro.sed)
;
; Template for relational operations on GPU. Used to auto-generate binary
; operation procedures using sed.
; Substitution tokens are: LE, LtEq and Compare two vetors for LTEQ
;
...
;       error status
;-
function gpuLtEq, p1, p2, p3, p4, p5, LHS=lhs, $
  ...
  case n_params() of
    2: begin
         ...
         if (x.n_elements ne y.n_elements) then begin
           ...
           message, level=-1, 'gpuLtEq: input vector length missmatch'
         endif

         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(*x.data LE *y.data)
         endif else begin
           case result_type of
             4 : err = gpuLtEqF(x.n_elements, x.handle, y.handle, _lhs.handle)
             5 : err = gpuLtEqD(x.n_elements, x.handle, y.handle, _lhs.handle)
             6 : err = gpuLtEqC(x.n_elements, x.handle, y.handle, _lhs.handle)
             9 : err = gpuLtEqZ(x.n_elements, x.handle, y.handle, _lhs.handle)
           endcase
         endelse
    ...
    5: begin
         ...
         if (!gpu.mode eq 0) then begin
           *_lhs.data = float(p1 * *x.data LE p3 * *y.data + p5)
         endif else begin
           case result_type of
             4 : err = gpuLtEqFAT(x.n_elements, $
                                         float(p1), x.handle, $
                                         float(p3), y.handle, $
                                         float(p5), $
                                         _lhs.handle)
             5 : err = gpuLtEqDAT(x.n_elements, $ 
                                         double(p1), x.handle, $
                                         double(p3), y.handle, $
                                         double(p5), $
                                         _lhs.handle)
             6 : err = gpuLtEqCAT(x.n_elements, $
                                         complex(p1), x.handle, $
                                         complex(p3), y.handle, $
                                         complex(p5), $
                                         _lhs.handle)
             9 : err = gpuLtEqZAT(x.n_elements, $
    ...
    else: message, level=-1, 'gpuLtEq: incorrect number of arguments'
  endcase
...
;       error status
;-
pro gpuLtEq, p1, p2, p3, p4, p5, p6, NONBLOCKING=nonblocking, ERROR=err
  on_error, 2


  case n_params() of
    3: p3 = gpuLtEq(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
                           LHS=p3, NONBLOCKING=nonblocking, ERROR=err)
    6: p6 = gpuLtEq(arg_present(p1) ? p1 : (size(p1, /n_dimensions) gt 0 ? p1[*] : p1[0]), $
                           arg_present(p2) ? p2 : (size(p2, /n_dimensions) gt 0 ? p2[*] : p2[0]), $
