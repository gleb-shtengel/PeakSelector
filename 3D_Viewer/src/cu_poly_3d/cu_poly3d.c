...
extern void poly3d(unsigned short *img, int nx, int ny, int nframes,
                   float *P, float *Q, unsigned short *out, int c1, int c2);
...
#define POLY_3D_CARDS_MUST_BE_UNIQUE -10
...
static IDL_MSG_DEF msg_arr[] = {  
  ...
  {"POLY_3D_CARDS_MUST_BE_UNIQUE", "%NCARD1 and CARD2 cannot both be the same"},
};
...
*  idl_poly3d
*/
static IDL_VPTR idl_poly3d(int argc, IDL_VPTR *argv, char *argk) {
  IDL_VPTR ans;
  unsigned short *img;
  ...
  int max_frames, card1, card2, ntesla, ncards;
  float *P, *Q;
  struct cudaDeviceProp info;
    ...
    if ((card1 != -1) && (card2 != -1) && (card1 == card2)) {
        IDL_MessageFromBlock(msg_block, POLY_3D_CARDS_MUST_BE_UNIQUE, IDL_MSG_LONGJMP);
    }
  ...
  for(k=0; k < nseg; k++) {
    ...
    if (n != 0) {
      poly3d(img+nelem*idx, xdim, ydim, n, P+2*2*idx, Q+2*2*idx, 
             (unsigned short *)out+nelem*idx, card1, card2);
    }
  }
...
int IDL_CDECL IDL_Load(void) {
  ...
  static IDL_SYSFUN_DEF2 function_addr[] = {  
    {{(IDL_FUN_RET) idl_poly3d}, "CU_POLY3D", 3, 3, IDL_SYSFUN_DEF_F_KEYWORDS, 0},
  };  
...
//dlm: VERSION 2.0
//dlm: FUNCTION CU_POLY3D 3 3 KEYWORDS
