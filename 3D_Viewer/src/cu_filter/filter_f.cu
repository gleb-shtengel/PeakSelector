...
*/
__device__ unsigned char check(float *row, int col) {
  float d = row[col];
...
*/
__global__ void k_gpfilter(float *gp, unsigned char *out) {
  int tid;
  float *row;
  unsigned char ans = 1;
...
*  out     -  Filter output vector
*/
__global__ void k_filter(float *gp, unsigned char *out) {
  int tid;
  float *row;
  unsigned char ans = 1;
...
*
*/
extern "C" void filter(float *gp, int npeaks, float *params,
                       unsigned char *out) {
  int nblocks;
  double s,e;
  ...
  cudaMalloc((void **)&d_gp, 45*nblocks*THREADS*sizeof(float));
  ...
  s = getTime();
  cudaMemcpy(d_gp, gp, 45*npeaks*sizeof(float), cudaMemcpyHostToDevice);
  cudaThreadSynchronize();
  e = getTime();
...
*
*/
extern "C" void gpfilter(float *gp, int npeaks, float *params,
                         unsigned char *out) {
  int nblocks;
  double s,e;
