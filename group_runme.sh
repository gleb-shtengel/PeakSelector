...
for i in `seq 0 $((NUM-1))`;
  ...
  # echo "source /usr/local/rsi/idl/bin/idl_setup.bash" 1>>${SCRIPT_NAME}${i}.sh old version of IDL
    ...
    qsub -l sl7=true -cwd -pe batch 8 -l d_rt=9999 -V -N ${SCRIPT_NAME}${i} -j y -o ${SCRIPT_NAME}${i}.out -b y -l idl_rt=6 ${PALM_DATA_DIR}/${TEMP_DIR}${SCRIPT_NAME}${i}.sh
...
qsub -l sl7=true -pe batch 8 -l d_rt=9999 -N last -o /dev/null -j y -b y -cwd -V -hold_jid $JOBLIST
