#!/bin/sh
PROG_NAME="ipalm_macro_worker.sav"
MONITOR_NAME="read_out_files.sav"
SCRIPT_NAME="ipalm_macro"
PALM_DATA_DIR=$2
IDL_SCR_DIR=$3
...
for i in `seq 0 $((NUM-1))`;
  ...
  # echo "source /usr/local/rsi/idl/bin/idl_setup.bash" 1>>${SCRIPT_NAME}${i}.sh old version of IDL
    ...
    bsub -n 2 -R"rusage[idl_rt=6]" -J ${SCRIPT_NAME}${i} -o ${SCRIPT_NAME}${i}.out idl_rt=6 ${PALM_DATA_DIR}/${TEMP_DIR}${SCRIPT_NAME}${i}.sh
