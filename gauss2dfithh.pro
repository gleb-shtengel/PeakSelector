...
Function Gauss2dfithh, z, a, x, y, NEGATIVE = neg, TILT=tilt, Sigma = sigma, CHISQ = chisq, FITA = fita, STATUS = status, ITMAX=itmax, YERROR=yerror
;+
; NAME:
;	GAUSS2DFIT
;
...
result = curvefit([nx, ny, x, y], reform(z, n, /OVERWRITE), $
		...
		function_name = "GAUSS2_FUNCT", FITA = fita, STATUS = status, YERROR=yerror)
;print,'1st fit a =',a
result = curvefit([nx, ny, x, y], reform(z, n, /OVERWRITE), $
		replicate(1.,n), a, sigma, ITMAX = itmax, CHISQ = chisq, $
		function_name = "GAUSS2_FUNCT", FITA = fita, STATUS = status, YERROR=yerror)
...
if Keyword_set(tilt) eq 0 then a = a $
